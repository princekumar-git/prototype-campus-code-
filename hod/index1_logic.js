...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function showSection(targetId) {
        ...
        // Only hide all sections if we have navigation items (multi-section SPA)
        // Don't hide if this is a standalone page with a single section
        if (navItems.length > 0) {
            sections.forEach(sec => sec.classList.add('hidden'));
        }
        const targetSec = document.getElementById(targetId);
        if (targetSec) targetSec.classList.remove('hidden');
        else console.error("Target section not found:", targetId);
        ...
        if (pageTitle) {
            const titleMap = {
                ...
                'section-manage-faculty': 'Faculty Management',
    ...
    navItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            e.stopPropagation(); // Stop event from bubbling to parent (important for nested items)
    ...
    const navUsersMenu = document.getElementById('navUsersMenu');
    const navUsersIcon = document.getElementById('navUsersIcon');

    if (navUsersBtn && navUsersMenu && navUsersIcon) {
        navUsersBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            navUsersMenu.classList.toggle('hidden');
            if (navUsersMenu.classList.contains('hidden')) {
                navUsersIcon.style.transform = 'rotate(0deg)';
            } else {
                navUsersIcon.style.transform = 'rotate(90deg)';
            }
        });
    }
    ...
    if (exportModal) {
        ...
        function scrapeAndMockFacultyData() {
            ...
            if (table) {
                // CASE 1: Table Found (Faculty Page)
                const rows = Array.from(table.querySelectorAll('tbody tr'));
                if (rows.length === 0) {
                    console.warn("No faculty rows found in the table.");
                }

                console.log(`Found ${rows.length} faculty rows.`);
                ...
                // CASE 2: No Table (Report Page) - Use Full Mock
                console.log("Faculty table not found. Generating full mock data for Report.");
            ...
            console.log("Scraped/Mocked data:", allFacultyData);
        }
        ...
        function renderPreviewTable() {
            ...
            if (!tableHead || !tableBody) {
                console.error("Critical Error: Export table elements not found in DOM.", { tableHead, tableBody });
                return;
            }
    ...
    if (studentExportModal) { // Check if modal exists (it will be in student.html and report.html)
        ...
        // NEW: Listener for Report Dropdown
        if (reportDropdown) {
        ...
        function scrapeAndMockStudentData() {
            ...
            if (table) {
                ...
                console.log("Student table not found (likely on Report page). Generating full mock data.");
    ...
    function initDashboardCharts() {
        ...
        if (ctxPerformance && ctxDifficulty) {
            ...
            // --- Dashboard Overall Report Logic ---
            ...
            if (btnDashboardReports) {
                btnDashboardReports.addEventListener('click', async () => {
                    ...
                    // --- ADD ANALYTICS CHART IF AVAILABLE ---
    ...
    try {
        ...
        console.error("Dashboard Charts Initialization Failed:", error);
    }
...
if (exportProblemsModal) {
    ...
    function scrapeAndMockProblemData() {
        ...
        console.log("Generating mock problem data for Report.");
...
if (exportContestModal) {
    ...
    function scrapeAndMockContestData() {
        console.log("Generating mock contest data for Report.");
        ...
        btnDownloadContestReport.addEventListener('click', () => {
            ...
            const htmlContent = `
                ...
                <body>
                    ...
                    <script>
                        setTimeout(() => {
                            // --- CHART 1: Verdict Distribution (Doughnut) ---
                            ...
                            // --- CHART 2: Submissions Over Time (Line) ---
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Ensure Profile Overlay Close Logic acts if not already independent
    const profileOverlay = document.getElementById('profileOverlay');
    ...
    if (profileOverlay) {
        if (closeProfileOverlay) {
            closeProfileOverlay.addEventListener('click', () => profileOverlay.classList.add('hidden'));
        }
        if (closeProfileBtnBottom) {
            closeProfileBtnBottom.addEventListener('click', () => profileOverlay.classList.add('hidden'));
        }
    }
});
